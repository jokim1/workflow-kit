...
- Repo-native commands are the source of truth. The AI can always fall back to `npm run pipelane:*` (the legacy `npm run workflow:*` aliases stay wired through the rename window).
...
a public `workflow:api` surface (the `workflow:api` command name stays stable across the rename so
...
Repo-native commands do not change. `npm run pipelane:*` is the canonical source of truth; `npm run workflow:*` stays wired as a deprecation alias for one release window.
...
npx pipelane init --project "Next Project"
npm run pipelane:setup
```
...
1. `npm install -D pipelane`
2. `npx pipelane init --project "Project Name"`
...
1. Pull the repo after the tracked workflow files are committed.
2. Run `npm run pipelane:setup` inside that repo.
