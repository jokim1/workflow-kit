...
import { homeClaudeDir, homeCodexDir, readJsonFile, resolveRepoRoot, runCommandCapture, writeJsonFile } from './state.ts';
...
export async function runUpdate(cwd: string, options: UpdateOptions): Promise<UpdateResult> {
  ...
  installLatest(repoRoot, { quiet: options.json, output: options.output });
...
function readFreshAutoUpdateCache(repoRoot: string, installedSha: string): AutoUpdateCache | null {
  ...
  if (!cache || cache.installedSha !== installedSha || !cache.latestSha || !cache.checkedAt) {
    return null;
  }
...
function installLatest(repoRoot: string, options: { quiet?: boolean; output?: UpdateOutputTarget } = {}): void {
